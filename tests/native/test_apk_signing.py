...
MATCHING_V2_APK = "V2 Signer: certificate SHA-1 digest: " + FINGERPRINT.replace(":", "").lower() + "\n"
...
class ApkSigningTests(unittest.TestCase):
    ...
    def test_accepts_scheme_prefixed_certificate_output_from_the_android_sdk(self):
        report = (
            "Verifies\nVerified using v2 scheme (APK Signature Scheme v2): true\nNumber of signers: 1\n"
            "V2 Signer: certificate DN: C=US, O=Android, CN=Android Debug\n"
            + MATCHING_V2_APK
            + "V2 Signer: public key SHA-1 digest: 98ce7aad82479a9a3f435adaa587fb5cfbbe5c27\n"
        )
        self.assertEqual(verifier.require_matching_certificate(KEY_REPORT, report), FINGERPRINT)
    ...
    def test_rejects_the_actual_mismatched_certificate_from_the_failed_google_sign_in_build(self):
        ...
        for report in [actual_report, actual_report.replace("Signer #1", "V2 Signer:")]:
            with self.subTest(report=report), self.assertRaisesRegex(ValueError, "APK signing SHA-1 mismatch"):
                verifier.require_matching_certificate(KEY_REPORT, report)

    def test_rejects_missing_or_additional_apk_signers(self):
        for report in ["", MATCHING_APK + MATCHING_APK.replace("#1", "#2"), MATCHING_V2_APK + MATCHING_V2_APK]:
